...
本目录与仓库内 **交互式分步教程**（`src/tutorial/`）一一对应：运行 `npm run dev` 后，在页面左侧切换第 01–09 课即可对照运行效果。文档用于离线阅读与检索 API、路径约定。
...
| 09 | [第 09 课 · 监听模型事件](./09-监听模型事件.md) | `#l09` | `cell:*` 事件、侧栏实时日志 |

在浏览器地址栏追加哈希可直接打开对应课程，例如：`http://localhost:5173/#l09`。
